...
class PayrollService {
    constructor(pool, payrollRepo, employeeRepo, auditRepo, taxService) {
        this.pool = pool;
    ...
    async runPayroll(params) {
        ...
        try {
            ...
            if (!run) {
                for (let i = 0; i < 10; i++) {
                    run = await this.payrollRepo.getRunByIdempotencyKey(client, params.idempotencyKey);
                    if (run)
                        break;
